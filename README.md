...
동작구 전지역 방문 관리(출장마사지·홈타이) 안내용 정적 사이트입니다.
예약전화: **0508-202-4719**

노원 사이트(nowon-massage)와 동일한 템플릿·디자인 시스템을 사용하며,
제작 규칙 전체는 `PLAYBOOK.md` 를 따릅니다.
...
```
build.py            # 빌드 스크립트 (레이아웃·TOC·글자수 검사·sitemap 생성)
content/
  site.py           # 상호·전화·BASE_URL·메뉴 구조
  main.py           # 메인 페이지 (+ LocalBusiness/FAQPage JSON-LD)
  areas.py          # 지역별: 동작구 허브 + 대표 동 8개
  stations.py       # 지하철역별: 허브 + 16개 역 (1·4·7·9호선, 신림선)
...
tools/gen_icons.py  # 브랜드 아이콘(G 모노그램) 재생성 스크립트
```
...
- 지역은 대표 동 8개만 (노량진동·상도동·본동·흑석동·동작동·사당동·대방동·신대방동)
  — 노량진1·2동, 상도1~4동, 사당1~5동, 신대방1·2동 등 숫자 행정동 페이지 없음
- 역은 역 1개당 페이지 1개 — 환승역(노량진·대방·동작·이수·보라매)도 URL 하나, 출구별 페이지 없음
